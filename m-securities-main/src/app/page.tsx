...
export default function Home() {
  ...
  const mmiFeatures = [
    { Icon: SparklesIcon, title: language === "mn" ? "AI шинжилгээ" : language === "zh" ? "AI分析" : "AI Analysis", desc: language === "mn" ? "Ухаалаг шинжилгээ, таамаглал" : language === "zh" ? "智能分析与预测" : "Smart insights & predictions" },
    { Icon: GlobeAltIcon, title: language === "mn" ? "Бүх мэдээлэл нэг дор" : language === "zh" ? "一站式数据" : "All-in-one Data", desc: language === "mn" ? "Хувьцаа, индекс, салбар, мэдээ" : language === "zh" ? "股票、指数、行业、新闻" : "Stocks, indices, sectors, news" },
    { Icon: BoltIcon, title: language === "mn" ? "Шуурхай, үнэн зөв" : language === "zh" ? "实时准确" : "Real-time Accurate", desc: language === "mn" ? "Бодит цаг дахь зах зээлийн мэдээлэл" : language === "zh" ? "实时市场数据" : "Live market data" },
    { Icon: TrophyIcon, title: language === "mn" ? "Топ компаниуд" : language === "zh" ? "优质公司排行" : "Top Companies", desc: language === "mn" ? "ROE, ашигт ажиллагааны дүн шинжилгээ" : language === "zh" ? "ROE及盈利能力分析" : "ROE & profitability analysis" },
  ];

  const miningProducts = [
    { Icon: ChartBarIcon, name: language === "mn" ? "Нүүрс" : language === "zh" ? "煤炭" : "Coal", tag: language === "mn" ? "Арилжаанд" : language === "zh" ? "交易中" : "Active", active: true },
    { Icon: ArrowTrendingUpIcon, name: language === "mn" ? "Зэсийн баяжмал" : language === "zh" ? "铜精矿" : "Copper Concentrate", tag: language === "mn" ? "Арилжаанд" : language === "zh" ? "交易中" : "Active", active: true },
    { Icon: BuildingLibraryIcon, name: language === "mn" ? "Төмрийн хүдэр" : language === "zh" ? "铁矿石" : "Iron Ore", tag: language === "mn" ? "Арилжаанд" : language === "zh" ? "交易中" : "Active", active: true },
    { Icon: GlobeAltIcon, name: language === "mn" ? "Бусад эрдэс" : language === "zh" ? "其他矿产" : "Other Minerals", tag: language === "mn" ? "Удахгүй" : language === "zh" ? "即将推出" : "Coming Soon", active: false },
  ];
    ...
    <div className={`min-h-screen transition-colors duration-300 ${bg}`}>
      ...
      <section className={`relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden pt-20
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6">
          ...
          <p className={`text-lg sm:text-xl mb-14 max-w-xl mx-auto leading-relaxed ${textSecondary}`}>
            {language === "mn" ? "Монголын болон дэлхийн хөрөнгийн зах зээлд хөрөнгө оруулах хамгийн найдвартай түнш."
              : language === "zh" ? "连接蒙古与全球资本市场，您最值得信赖的投资伙伴。"
            ...
            {services.map(s => (
              <Link key={s.href} href={s.href}
                className={`group rounded-2xl border p-6 block transition-all duration-300 hover:-translate-y-1
                  ${cardBg} ${cardBorder}
                  ${isDarkMode ? "hover:bg-[#151820] hover:border-teal-500/20 hover:shadow-lg hover:shadow-teal-500/5"
                    : "hover:border-teal-200 hover:shadow-lg hover:shadow-teal-500/8"}`}>
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className={`text-base leading-relaxed mb-8 ${textSecondary}`}>
                  {language === "mn" ? "Хувьцаа, индекс, салбарын гүйцэтгэл, санхүүгийн мэдээллийг бодит цаг дээр хянаж, илүү ухаалаг хөрөнгө оруулалтын шийдвэр гаргаарай."
                    : language === "zh" ? "实时跟踪股票、指数、行业表现和财务数据，做出更明智的投资决策。"
          ...
          <div className={`rounded-3xl border p-10 sm:p-14 relative overflow-hidden ${cardBg} ${cardBorder}`}>
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className={`text-base leading-relaxed mb-8 ${textSecondary}`}>
                  {language === "mn" ? "М Секьюритис ҮЦК нь Монголын Хөрөнгийн Биржээр дамжуулан нүүрс, зэсийн баяжмал болон бусад уул уурхайн бүтээгдэхүүний арилжааг мэргэжлийн түвшинд хэрэгжүүлдэг."
                    : language === "zh" ? "M Securities 通过蒙古证券交易所专业开展煤炭、铜精矿等矿产品交易。"
              ...
              <div className="grid grid-cols-2 gap-3">
                {miningProducts.map(p => (
                  <div key={p.name} className={`rounded-2xl border p-5 text-center transition-all hover:-translate-y-1
                    ${isDarkMode ? "bg-white/3 border-white/6 hover:border-teal-500/20" : "bg-gray-50 border-gray-100 hover:border-teal-200 hover:shadow-sm"}`}>
      ...
      <section className={`py-24 px-6 ${sectionAlt}`}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h2 className={`text-3xl sm:text-4xl font-bold mb-6 ${textPrimary}`}>{t("home.about.title")}</h2>
          <p className={`text-base sm:text-lg leading-relaxed mb-10 ${textSecondary}`}>{t("home.about.description")}</p>
      ...
      <section className={`py-24 px-6 ${bg}`}>
        <div className="max-w-3xl mx-auto text-center">
          <h2 className={`text-4xl sm:text-5xl font-bold mb-6 leading-tight ${textPrimary}`}>{t("home.cta.title")}</h2>
          <p className={`text-lg mb-10 ${textSecondary}`}>{t("home.cta.description")}</p>
          <a href="https://trader.msecurities.mn/" target="_blank" rel="noopener noreferrer"
